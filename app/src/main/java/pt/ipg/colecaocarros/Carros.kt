...
data class Carros(var marca: String,var modelo: String, var data: Date, var id_detalhes: Long, var id: Long=-1) {
    fun toContentValues():ContentValues{
        val valores = ContentValues()

        valores.put(TabelaCarro.CAMPO_MARCA, marca)
        valores.put(TabelaCarro.CAMPO_MODELO, modelo)
        valores.put(TabelaCarro.CAMPO_DATA, data.time)
    ...
    companion object{
        fun fromCursor(cursor: Cursor): Carros{
            ...
            val data = Date(cursor.getLong(posData))
